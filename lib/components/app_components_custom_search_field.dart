...
class CustomSearchField<T> extends StatefulWidget {
  ...
  @override
  _CustomSearchFieldState<T> createState() => _CustomSearchFieldState();
}
...
class _CustomSearchFieldState<T> extends State<CustomSearchField<T>> {
  ...
  void initialize() {
    ...
    _focus?.addListener(() {
      if (mounted) {
        setState(() {
          isSuggestionExpanded = (_focus?.hasFocus ?? false);
        });
        ...
        if (isSuggestionExpanded) {
          if (widget.initialValue == null) {
            if (widget.suggestionState == Suggestion.expand) {
              Future.delayed(const Duration(milliseconds: 100), () => suggestionStream.sink.add(widget.suggestions));
            }
          }
        }
      }
